...
<template>
	<view class="activityinput">
		...
		<view class=" img-top-bg">
			<view v-if="!posterurl" class="add-head-pic inline" @click="uploadpic1">点击添加活动封面（必填）</view>
			<image class="img-top" v-else :src="posterurl" @click="uploadpic1" mode="aspectFill"></image>
		</view>
		...
		<view class="inline ac-cont">
			...
			<view class="inline-one inline-one-jianjie" style="display:block;">
				...
				<jin-edit placeholder="请输入内容" @editOk="editOk" :html="html"></jin-edit>
				<!-- <view class="cont">
		...
		<view class="sub-button" @click="subactivity">编辑发布</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			async getActivity(){//得到活动详情
				let {mid} = this;
				try {
					var data = {mid:mid}
				  let response = await this.$http.post('/activity/editActivity',data,'');
				  if (response.rspInfo.rspCode == 1000) {
						this.tags=response.rspData.tags;
						this.activity_name=response.rspData.name;
						this.posterurl=response.rspData.logo;
						this.startime=response.rspData.starttime;
						this.endtime=response.rspData.endtime;
						this.bstartime=response.rspData.entrystarttime;
						this.bendtime=response.rspData.entryendtime;
						this.city=response.rspData.city;
						this.html=response.rspData.matchdetail.match_match_Field[0].content;
						this.price=response.rspData.price;
						this.toplimit=response.rspData.toplimit;
						this.sponsor=response.rspData.sponsorname;
						this.sponsor1=response.rspData.sponsor;
						this.phone=response.rspData.phone;
						this.youselfItem = response.rspData.userinfo;
						// this.selfItems = response.rspData.userextra;
						this.selfItems=response.rspData.userextra.map((r,index) => {
							var index = this.selftype.indexOf(r.type)+1;
							var data = {'type':{index: index},cont:r};
							return data
						})
				  }else{
				    uni.showToast({
			...
			async subactivity(){
				let {sponsor1, mid, tags, posterurl, activity_name, startime, endtime, bstartime, bendtime, city, html, price, toplimit, sponsor, phone} = this;
				...
				try {
				  let response = await this.$http.post('/activity/updateActivity',data);
				  if (response.rspInfo.rspCode == 1000) {
						uni.showToast({
							title: '更新发布成功',
							duration: 1500
						});
						if(uni.getStorageSync("channel")=='cmmc'){
							setTimeout(function(){
								uni.navigateTo({
									url:'../myacvitity/myacvitity'
								})
							}, 1500)
						}
